"""Selective-RMSE evaluation shared by every method in experiments2 (proposal
eq. 2-3). Ported from experiments/evaluate.py, importing the local
experiments2/abstention.py instead of reaching into experiments/.

Every baseline/method script ends up calling `sweep(...)` once per abstention
rule (or learned uncertainty score) it wants to score, and the results (one
dict per row) are exactly what a Table-1-style comparison is assembled from.
"""
...
def sweep(
    ...
    a learned Uhat/W, or the score from a baseline abstention rule."""
